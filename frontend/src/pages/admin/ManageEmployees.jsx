...
export default function AdminManageEmployees() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUserId) {
        await updateUser(editingUserId, { ...userForm, role: 'Employee' })
        notify('Employee updated successfully')
      } else {
        await createUser({ ...userForm, role: 'Employee' })
        notify('Employee created successfully. Default password is password123')
      }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">Create and manage employees.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <SectionCard title={editingUserId ? 'Edit Employee' : 'Create Employee'} subtitle="Add a new employee to the platform.">
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {isSubmitting ? (editingUserId ? 'Updating...' : 'Creating...') : (editingUserId ? 'Update employee' : 'Create employee')}
              </button>
              {editingUserId ? (
                <button
                  type="button"
        ...
        <SectionCard title="Employees Directory" subtitle="All employees in the system.">
          <div className="space-y-4 max-h-[600px] overflow-auto pr-1">
            {bootstrap.employees.map((person) => {
              ...
              return (
                <div key={person.id} className="rounded-lg border border-gray-100 p-3 mb-2 flex justify-between items-center hover:bg-gray-50 transition-colors">
                  ...
                  <div className="text-right flex flex-col items-end gap-2">
                    <span className="inline-block px-2 py-1 rounded-full text-[10px] font-bold tracking-wide uppercase bg-gray-100 text-gray-600">
                      Employee
                    </span>
            ...
            {bootstrap.employees.length === 0 && !loading ? (
